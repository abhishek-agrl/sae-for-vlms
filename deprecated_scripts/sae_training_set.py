...
class ActivationDataset(Dataset):
    def __init__(self, h5_path, layer_name):
        ...
        # Verify the requested layer exists in the HDF5 file
        if layer_name not in self.h5_file:
            available_layers = list(self.h5_file.keys())
            raise ValueError(f"Layer '{layer_name}' not found. Available layers: {available_layers}")
            
        self.data = self.h5_file[layer_name]
        
    def __len__(self):
        return self.data.shape[0]

    def __getitem__(self, idx):
        # Data is saved as float16, this converts it to float32 for training
...
def train_sae(dataloader, model, optimizer, layer_name):
    ...
    torch.save(model.state_dict(), f'TopKSAE_training_set_{layer_name}.pth')
...
if __name__ == "__main__":
    ...
    data_path = "/home/abhishek.agrawal/vlm/llava_multiple_layers.h5"
    cfg['device'] = "cuda" if torch.cuda.is_available() else "cpu"
    layer_to_train = 'layer_22' # Available: 'layer_11', 'layer_17', 'layer_22', 'layer_23'
    ...
    train_sae(dataloader=dataloader, model=model, optimizer=optimizer, layer_name=layer_to_train)
